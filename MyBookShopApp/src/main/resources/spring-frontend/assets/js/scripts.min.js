...
(function ($) {
    ...
    $(function () {
            ...
            return {
                init: function () {
                    ...
                    $input.on('blur', function () {
                        ...
                        var $this = $(this), validate = $this.data('validate'), message = '', error = false, lengthCode = $this.data('lengthcode'), locale = $this.data('locale') ;
                        console.log("lengthCode", lengthCode)
                        if (validate) {
                            ...
                            validate.forEach(function (v) {
                                switch (v) {
                                    case'require':
                                        ...
                                        if ((!$this.val() || $this.val().includes("x")) && !$this.prop('disabled')) {
                                            message += (locale === "ru" ? 'Это поле обязательно для заполнения.' : 'This field is required.');
                                            error = true;
                                        }
                                        break;
                                    case'mail':
                                        if ($this.val() !== '' && !$this.val().match(/\w+@\w+\.\w+/) && !$this.prop('disabled')) {
                                            message += (locale === "ru" ? 'Нужно ввести адрес почты в формате xxx@xxx.xx' : 'You need to enter your email address in the format xxx@xxx.xx');
                                            error = true;
                                            console.log (locale === "ru" ? 'Это поле обязательно для заполнения.' : 'This field is required.')
                                        }
                                        break;
                                    case'key':
                                        if ($this.val() !== '' && !$this.val().replace(' ', '').match(new RegExp("\\d{" + lengthCode + "}")) && !$this.prop('disabled')) {
                                            message += (locale === "ru" ? 'Код должен состоять из ' + lengthCode + ' цифр' : 'The code must consist of ' + lengthCode + ' digits');
                                            error = true;
                                            console.log (locale === "ru" ? 'Это поле обязательно для заполнения.' : 'This field is required.')
                                        }
                                }
                                ...
                                $.ajax({
                                    ...
                                    complete: function (result) {
                                        if (result.status === 200) {
                                            ...
                                            result.responseJSON.data.books.forEach(function (book) {
                                                data += Card().bookTemplate(book);
                                            });
                                            var $Cards = $('.Cards_refresh');
                                            let countBook = result.responseJSON.data.totalCount
                                            $Middle.find('.Middle-searchHeader').text('Найден' + (countBook % 10 == 1 && countBook % 100 != 11 ? 'а ' : 'о ')  +
                                                countBook + ' книг' +
                                                (countBook % 10 === 1 && countBook % 100 != 11 ? 'a' : (countBook % 10 > 1 && countBook % 10 < 5 && (countBook < 10 || countBook > 20) ? 'и' : '') ))
                    ...
                    $('[data-refreshshow]').on('click change', function (e) {
                            ...
                            $.ajax({
                                ...
                                complete: function (result) {
                                    if (result.status === 200) {
                                        ...
                                        changeIndexForLinkCards()
                                        console.log("result.responseJSON: ", result.responseJSON)
                                        if (result.responseJSON.data.endList){

                                            document.querySelector('a.btn.btn_primary').style.display = 'none'
                                            if (document.querySelectorAll('div.Card').length === 0){
                                                document.querySelector('div.Cart-messageInfo').style.display = 'block'
                                            }
                                        } else {
                                            document.querySelector('a.btn.btn_primary').style.display = 'inline-flex'
                                            document.querySelector('div.Cart-messageInfo').style.display = 'none'
                                        }
                                    } else {
                                        alert('Ошибка ' + result.status);
                                    }
                        ...
                        switch (data) {
                            ...
                            case'tag':
                                ...
                                break;
                            case'my':
                                getData('/api/books/my', {
                                    offset: $this.data('refreshoffset'),
                                    limit: $this.data('refreshlimit')
                                });
                                break;
                            case'archive':
                                getData('/api/books/my/archive', {
                                    offset: $this.data('refreshoffset'),
                                    limit: $this.data('refreshlimit')
                                });
                                break;
                            case'history':
                                getData('/api/books/my/history', {
                                    offset: $this.data('refreshoffset'),
                                    limit: $this.data('refreshlimit')
                                });
                                break;
                            case'search':
                                console.log("document.querySelector(\"#querypage\").value", document.querySelector("#querypage").value)
                                getData('/api/search/' + document.querySelector("#querypage").value, {
                                    offset: function () {
                                        var oldSearchText = $this.data('refreshquery')
                                        if (document.querySelector("#querypage").value != oldSearchText){
                                            $this.data('refreshquery', document.querySelector("#querypage").value)
                ...
                init: function () {
                    ...
                    $('#submitPhone').on('click', function () {
                        var $this = $(this), $row = $this.closest('.form-group_row'),
                            data = {contact: $row.find('.form-input_phone').val(), login: false};
                    ...
                    $('#submitMail').on('click', function () {
                        var $this = $(this), $row = $this.closest('.form-group_row'),
                            data = {contact: $row.find('.form-input_mail').val().toLowerCase(), login: false};
                        postData('/requestContactConfirmation', data, function (result) {
                            if (result.result) {
                                console.log("3")
                    ...
                    $Registration.find('.form-input_code').on('blur', function () {
                        var $this = $(this), $row = $this.closest('.form-group_row'), data = {
                            contact: $row.find('.form-input_mail, .form-input_phone').val().toLowerCase(),
                        ...
                        if (!$this.hasClass('form-input_error')) {
                            postData('/approveContact', data, function (result) {
                                if (result.result) {
                                    ...
                                    $but.after('<div class="form-info">' + result.message + '</div>');
                                    $this.trigger('change');
                                } else {
                                    if (result) {
                    ...
                    $sendButton.on('click', function (e) {
                        ...
                        if ($this.is('[data-send]')) {
                            ...
                            $this.data('send', {contact: contact.toLowerCase(), login: true});
                            if (!error) {
                    ...
                    $entryButton.on('click', function (e) {
                        ...
                        if ($this.is('[data-send]')) {
                            ...
                            $this.data('send', {code: code, contact: send.contact.toLowerCase()});
                                ...
                                postData('/login', $this.data('send'), function (result) {
                                    if (result != null && result.result) {
                                        ...
                                        if (result != null && result.result != null) {
                                            ...
                                            $thisStep.find('.form-input:not(.form-input_hide)[type="text"]').addClass('form-input_error').after('<div class="form-error">' + result.message + '</div>');
                                        }
                                    }
                                })
                            } else {
            ...
            return {
                init: function () {
                    ...
                    $btnSend.on('click', function (e) {
                        ...
                        let books = String($this.data('bookid')).split(",")
                                ...
                                if (result.result) {
                                    ...
                                    if ($cart.length) {
                                        ...
                                        $cart.find('.Cart-product').each(function () {
                                            ...
                                            priceOld += parseFloat($this.find('.Cart-price_old').text()) ? parseFloat($this.find('.Cart-price_old').text()) : 0;
                                    ...
                                    setTimeout(e=> {
                                        document.querySelector('.btnError').remove()
                                    }, 2500)
                    ...
                    $ProductAssesment.find('.Rating input').on('change', function (e) {
                        ...
                        console.log("jq ",$this.prop('checked'))
                        Login().postData('/api/rateBook/' + $this.closest('.Rating').data('bookid'), {
                                    ...
                                    if ($(slick.$slider).data('load')) {
                                        ...
                                        switch ($(slick.$slider).data('load')) {
                                            ...
                                            case'popular':
                                                ...
                                                break;
                                            case'author':
                                                sendData('/api/books/author/' + $this.data('refreshid'))
                                                break;
                                        }
                                    }
                                }
        ...
        if (document.location.search.indexOf('result') >= 0) {
            ...
            elems[2].click()
            let msg = document.querySelector('.Topup-success');
            if (document.location.search.indexOf('result=true') >= 0) {
                msg.textContent = 'Оплата прошла успешно';
                msg.style.display = 'block'

            } else if (document.location.search.indexOf('result=false') >= 0) {
                msg.textContent = 'Во время оплаты произошла ошибка';
                msg.style.color = '#ba6347'
                msg.style.display = 'block'
            } else if (document.location.search.indexOf('result=error') >= 0) {
                msg.textContent = 'Недостаточно средств, пожалуйста пополните баланс';
                msg.style.color = '#ba6347'
                msg.style.display = 'block'
            }
            setTimeout(() => {
            ...
            function getData(address, data) {
                ...
                $.ajax({
                    ...
                    complete: function (result) {
                        if (result.status === 200) {
                            var data = '';
                            console.log("trans ", result.responseJSON)
                            result.responseJSON.data.transactions.forEach(function (transaction) {
                                data += transactionTemplate(transaction);
                            });
                            $('.Transactions tbody').append(data);
                            let btn = document.querySelector('.Transactions-get')

                            $('.Transactions-get').data('transactionoffset', $('.Transactions-get').data('transactionoffset') +1 )
                            if (result.responseJSON.data.endList){
